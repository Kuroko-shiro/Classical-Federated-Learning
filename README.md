...
**現状（2026-07）：実データでの古典ベンチマークは完了。**
2×2（モデル同種/異種 × パラメタ/ロジット経路）が全セル確定し、
②の経路走査（HeteroFL / FedMD / FedProto）も完了。
残るは ② の α スイープと ④ への HeteroFL 移植。
...
## ⚠️ 最重要：既定値はランナー間で揃っていない

**`--test-subset` の既定値がランナーごとに違う。** テスト集合は全 627 件で、
コード上 `test_idx = test_idx_full[:test_subset]`（先頭からの切り出し）。

| ランナー | `--test-subset` 既定 | 実効評価件数 |
|---|---|---|
| `iu_federated.py`（①②） | 300 | 300 |
| `iu_federated_s3.py`（③） | **700** | **627（全件）** |
| `iu_federated_s4.py`（④） | 300 | 300 |
| `iu_baselines.py` | 300 | 300 |

過去の③ランは `--test-subset` を明示していないため **627 件で評価**されており、
①②④・ベースライン（300 件）と**評価集合が異なる**。

- ③内部の比較（fedavg / min / loot / uniml）は全て 627 件で揃っており無傷。
- ①②④内部の比較（2×2 を含む）も全て 300 件で揃っており無傷。
- **③の gap_filled と ③→④ の跨ぎ比較のみ、分子と分母で評価集合が混在**する。

**規律：以降は全ランで全パラメタを明示的に渡すこと。** 既定値に依存しない。
特に `--test-subset` `--train-subset` `--local-epochs` `--batch` `--rounds` は必ず書く。

学習はテスト集合に依存しないため、**評価集合差の検証に再学習は不要**。
同一設定・同一シードで短縮ラン（6ラウンド）を回し、既存CSVの同ラウンドと
AUROC を突き合わせれば、差はそのまま評価集合の効果になる（決定性は検証済み）。
...
| `iu_federated_s4.py` | ④（複合異種） | `fedmd` `fedmd_loot` |
...
caffeinate -dimsu python scripts/iu_federated.py \
    ...
    --rounds 40 --train-subset 2670 --test-subset 300 \
...
caffeinate -dimsu python scripts/iu_federated_s3.py \
    ...
    --clients 4 --rounds 40 \
    --train-subset 2670 --test-subset 300 \
    --local-epochs 2 --batch 8 --num-workers 2 \
    2>&1 | tee logs/s3_loot_1-3_a0.1.log
```

> 過去ランは `--test-subset` を省略していた（＝627件評価）。新規ランは明示すること。

### ④ 複合異種

`scripts/iu_federated_s4.py --method fedmd|fedmd_loot --mm-ratio 1:3 --alpha 100.0`（他は同様）
...
caffeinate -dimsu python scripts/iu_baselines.py \
    ...
    --epochs 80 --eval-every 2 --train-subset 2670 --test-subset 300 --batch 8
```

**Local は α ごとに取り直す必要がある**（Dirichlet 分割が α で変わるため）。
α の効果は +0.117、幅異種の効果は +0.027 と実測されており、**使い回し不可**。

Centralized 上限は `iu_baselines.py --mode centralized` ではなく、
**FLハーネスを `--clients 1 --all-data` で回す**（下記「上限の定義」参照）。
...
| Local基準線 | `--epochs 80 --eval-every 2`（＝40評価点）、**クライアント毎ピークの平均** |
| 主指標 | macro-AUROC |
| 報告 | **peak と conv（R14–39平均）の二重報告**（FedMD系の終盤劣化のため） |
| durable判定 | Δ = peak − conv。Δ≈0.01 は堅牢、Δ=0.05〜0.15 は一過性 |
| 異種モデルの評価 | 集約後に各自の幅で再参加 → **per-client 平均** |

**上限の定義（B案）**：`iu_baselines.py --mode centralized` は epoch 5 でピーク後に
崩壊し（0.9514 → conv 0.708）、FL 6ランがこれを超過したため**上限として不成立**。
`--clients 1 --all-data` で FL ハーネス・同一スケジュールに揃えて再走しても 0.9514 止まり。
そこで**経験上限 C = 0.9748（① FedAvg IID full-participation の peak）**を採用する。
これは「プール逐次学習 < 周期平均化」の直接証拠でもある（local SGD の正則化効果と整合）。
...
## 実データ結果（IU X-ray, 4 clients, C = 0.9748）
...
- `iu_federated.py` は CSV 横に `<csv>.config.json`（全args + git hash）を自動出力する。
  **`s3` / `s4` / `baselines` には未実装** → 追加すること。
