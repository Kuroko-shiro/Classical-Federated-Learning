...
class WindowDataset(Dataset):
    def __init__(self, tensor_path: Path, manifest: pd.DataFrame, condition: str):
    ...
    def __getitem__(self, i):
        ...
        if self.condition == "ecg_dual":
            s = x[0:1]
        elif self.condition == "pleth_dual":
            s = x[1:2]
        else:
            raise ValueError(self.condition)
        x = np.concatenate([s, s], axis=0)
...
class DualBranchSameSensorModel(nn.Module):
    """Exact architecture/parameter-count control for the ECG+PLETH fusion model.

    Both branches receive the same sensor. If ECG+PLETH still beats this control,
    the gain cannot be attributed solely to having two encoders / more parameters.
    """

    def __init__(self, latent=64):
        super().__init__()
        self.a = Encoder(latent)
        self.b = Encoder(latent)
        self.head = nn.Sequential(
            nn.Linear(2 * latent, 64), nn.ReLU(), nn.Dropout(0.1), nn.Linear(64, 1)
        )

    def forward(self, x):
        a = F.normalize(self.a(x[:, 0:1]), dim=1)
        b = F.normalize(self.b(x[:, 1:2]), dim=1)
...
def metrics(y, p, thr):
    ...
    return dict(
        auprc=auprc, auroc=auroc, f1=f1, sensitivity=sens, specificity=spec,
        ppv=ppv, prevalence=float(np.mean(y)), n_windows=int(len(y))
    )
...
def main():
    ...
    ap.add_argument("--condition", choices=["ecg_dual", "pleth_dual"], required=True)
    ...
    trds = WindowDataset(tensor, train, args.condition)
    vds = WindowDataset(tensor, val, args.condition)
    tds = WindowDataset(tensor, test, args.condition)
    g = torch.Generator()
    g.manual_seed(args.seed)
    tr = DataLoader(trds, batch_size=args.batch_size, shuffle=True, num_workers=2,
                    generator=g, persistent_workers=True)
    va = DataLoader(vds, batch_size=args.batch_size, shuffle=False, num_workers=2,
                    persistent_workers=True)
    te = DataLoader(tds, batch_size=args.batch_size, shuffle=False, num_workers=2,
                    persistent_workers=True)

    device = torch.device("cpu")
    model = DualBranchSameSensorModel().to(device)
    nparams = sum(p.numel() for p in model.parameters())
    if nparams != 55681:
        raise RuntimeError(f"capacity control must exactly match fusion params; got {nparams}")
    ...
    for epoch in range(args.epochs):
        ...
        vy, vp = predict(model, va, device)
        vap = float(average_precision_score(vy, vp))
        vau = float(roc_auc_score(vy, vp))
        row = {"epoch": epoch, "train_loss": float(np.mean(losses)), "val_auprc": vap, "val_auroc": vau}
        history.append(row)
    ...
    vy, vp = predict(model, va, device)
    thr, _ = threshold_from_val(vy, vp)
    ty, tp = predict(model, te, device)
    vm = metrics(vy, vp, thr)
    tm = metrics(ty, tp, thr)

    result = {
        "condition": args.condition,
        "sensor_capacity_control": True,
        "architecture": "two independent encoders + fusion head; identical selected sensor duplicated to both branches",
        "seed": args.seed,
